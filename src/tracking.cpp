...
vector<vector<Point2f>> Tracking::visualizeGoodKeypoints(vector<vector<KeyPoint>> &list_keypoints_dataset, vector<Mat> &list_descriptors_dataset, vector<KeyPoint> &keypoints_frame, Mat &descriptors_frame, vector<Mat> &H) {
...
void Tracking::drawRect(vector<Mat> H, vector<vector<Point2f>> vec_corners) {
...
//    obj_corners[3] = Point2f(0, static_cast<float>(images_dataset.at(0).rows));
    ...
    for (int i=0; i<H.size(); i++) {
        ...
        cout<<"----------"<<endl<<"Detected object n. " + to_string(i+1)<<endl<<"Press a key"<<endl;
        waitKey(0);
    }
}
...
void Tracking::trackObjects(vector<vector<Point2f>> allcoords_keypoints) {
    ...
    vector<Mat> final_frame(size_video);
    vector<Scalar> color = {Scalar(0,0,255), Scalar(255,0,0), Scalar(0,255,0), Scalar(50,50,50)};
...
//    }
    ...
    for (int i = 1; i < images_frame.size(); i++) {
        ...
        for (int j = 0; j < images_dataset.size(); j++){
            ...
            cornerSubPix(first_image_gray, allcoords_keypoints.at(j), Size(win_size, win_size), Size(-1,-1), termcrit);
            calcOpticalFlowPyrLK(first_image_gray, gray, allcoords_keypoints.at(j), nextPoints, status, err, Size( win_size*2+1, win_size*2+1 ), 5, termcrit);
            
            allNextPoints.push_back(nextPoints);
        
            for (int k = 0; k < allcoords_keypoints.at(j).size(); k++){
...
//            for (int h = 0; h < allcoords_keypoints.at(j).size(); h++){
//
//                if (!status[h]){
//
//                    allcoords_keypoints.at(j)[h] = allNextPoints.at(j)[h];
//                }
//            }
        }
